...
**Build health (2026-06-07):** `cargo test --workspace` → **123 passed, 0 failed** (+ doctests incl. a multi-socket compile-fail); `fmt`/`clippy -D warnings`/`build --release`/`doc`/`deny`/`audit` all clean. **14 crates exist** (the new L6 `systemrs-twin` added in M6); only `systemrs-ffi` from the §10 plan remains unbuilt.
