...
**Build health (2026-06-06):** `cargo test --workspace` → **111 passed, 0 failed** (+ doctests incl. a multi-socket compile-fail); `fmt`/`clippy -D warnings`/`build --release`/`doc`/`deny`/`audit` all clean. **13 of 14** planned crates exist (`systemrs-tlm1` + `systemrs-trace` added in M5; only `systemrs-ffi` remains).
