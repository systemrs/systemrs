...
//! - [`rv32i`] — an RV32IM CPU hart: an `SC_THREAD` that fetches, decodes, and executes
//!   the RV32I base integer instruction set plus the M (multiply/divide) extension, with
//!   a minimal machine-mode CSR/trap unit; all memory access goes through `b_transport`
//!   over an initiator socket.
//! - [`soc`] — a small system-on-chip: the [`rv32i`] hart driving an address-decoding
//!   interconnect to RAM, a UART, and a CLINT-like timer whose level-sensitive interrupt
//!   the hart takes; runs bare-metal firmware and is deterministic across runs.
